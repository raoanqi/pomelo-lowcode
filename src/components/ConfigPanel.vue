          ...
          <div class="config-section">
            <div v-for="prop in getConfigurableProps" :key="prop.key" class="config-item">
              ...
              <div class="config-value">
                <!-- 输入框 -->
                <el-input v-if="prop.type === 'input'" v-model="currentProps[prop.key]"
                  @change="updateProps(prop.key, $event)" size="small" />

                <!-- 选择器 -->
                <el-select v-else-if="prop.type === 'select'" v-model="currentProps[prop.key]"
                  @change="updateProps(prop.key, $event)" size="small" style="width: 100%">
                  <el-option v-for="option in prop.options" :key="option.value" :label="option.label"
                    :value="option.value" />
                </el-select>

                <!-- 开关 -->
                <el-switch v-else-if="prop.type === 'switch'" v-model="currentProps[prop.key]"
                  @change="updateProps(prop.key, $event)" />

                <!-- 选项编辑器 -->
                <div v-else-if="prop.type === 'options'">
                  <div v-for="(option, index) in currentProps.options" :key="index" class="option-item">
                    ...
                    <el-button type="danger" size="small" icon="Delete"
                      @click="currentProps.options.splice(index, 1); handleOptionsEdit(currentProps.options)" />
                  </div>
                  <el-button type="primary" size="small"
                    @click="currentProps.options.push({ label: '新选项', value: 'new' }); handleOptionsEdit(currentProps.options)">
                    添加选项
                  </el-button>
                </div>
              </div>
        ...
        <el-tab-pane label="样式">
          <div class="config-section">
            <div v-for="style in configurableStyles" :key="style.key" class="config-item">
              ...
              <div class="config-value">
                <!-- 颜色选择器 -->
                <el-color-picker v-if="style.type === 'color'" v-model="currentStyle[style.key]"
                  @change="updateStyle(style.key, $event)" size="small" />

                <!-- 输入框 -->
                <el-input v-else v-model="currentStyle[style.key]" @change="updateStyle(style.key, $event)"
                  size="small" />
              </div>
            </div>
          </div>
